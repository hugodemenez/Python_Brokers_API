...
class binanceV2:
    ...
    def price(self,symbol):
        symbol = self.symbol_format(symbol)
        for pair in self._get("ticker/bookTicker"):
            if pair['symbol']==symbol:
                return {'bid':pair['bidPrice'],'ask':pair['askPrice']}
        raise Exception("No matching symbol found")
...
if __name__=='__main__':
    broker = binanceV2()
    symbol = 'BTC*USDT'
    print(broker.get_balances(symbol))
